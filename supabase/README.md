...
### 쿠폰함·특별쿠폰 (202609150007)
기존 멤버십 설치 후 007 SQL을 실행하고 admin-api를 재배포합니다. 회원별 월 기본쿠폰과 특별쿠폰의 사용 가능/확보/사용/만료 상태를 제공합니다. 관리자는 활성 회원에게 5천 원, 00~09시, 처음 최대 3시간 쿠폰을 유효기간과 함께 반복 발급합니다. 예약당 한 장, 등급 할인 이후 적용, 최종 금액 0원 하한입니다. 서버가 소유권·만료·중복사용을 검사하고 활성 예약 unique index로 보호합니다. 예약 가격은 스냅샷이며 취소 시 유효기간 안에서 재사용합니다.

멤버 예약 캘린더·관리자 예약 캘린더·정산 월 조회는 이전/다음 월 버튼으로 이동합니다. 정산 기간 12일~다음 달 11일은 유지합니다.
