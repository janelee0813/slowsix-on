-- Add administrator-only membership removal, preserving booking history and prior approval rules.
begin;
alter table ss_admin.members add column if not exists deleted_at timestamptz;
create table if not exists ss_admin.special_coupons (
 id uuid primary key, member_id uuid not null references ss_admin.members(person_id),
 kind text not null check(kind in ('discount5000','night','hours3')),
 expires_at timestamptz not null, created_at timestamptz not null default now(),created_by uuid not null references ss_admin.people(id)
);
alter table ss_admin.special_coupons enable row level security;
revoke all on ss_admin.special_coupons from public,anon,authenticated;
alter table ss_admin.bookings add column if not exists special_coupon_id uuid references ss_admin.special_coupons(id);
create unique index if not exists special_coupon_once on ss_admin.bookings(special_coupon_id) where special_coupon_id is not null and status in ('requested','awaiting_payment','confirmed');
create or replace function ss_admin.special_coupon_price(cp uuid,person uuid,st timestamptz,en timestamptz,guests int,price jsonb)
returns jsonb language plpgsql stable set search_path=pg_catalog,ss_admin as $$
declare c ss_admin.special_coupons; waived bigint; discount bigint;
begin
 select * into c from ss_admin.special_coupons where id=cp and member_id=person and expires_at>now();
 if not found or exists(select 1 from ss_admin.bookings where special_coupon_id=cp and status in ('requested','awaiting_payment','confirmed')) then raise exception 'COUPON_UNAVAILABLE'; end if;
 if c.kind='discount5000' then discount:=5000;
 else
  select coalesce(sum(case when extract(isodow from h at time zone 'Asia/Seoul') in (6,7) then 2500 else 2000 end)*guests,0) into waived from generate_series(st,en-interval '1 hour',interval '1 hour') h
  where (c.kind='night' and extract(hour from h at time zone 'Asia/Seoul')<9) or (c.kind='hours3' and h<st+interval '3 hours');
  discount:=waived*(100-(price->>'discount_percent')::int)/100;
 end if;
 if discount<=0 then raise exception 'COUPON_NOT_APPLICABLE'; end if;
 discount:=least(discount,(price->>'total_amount')::bigint);
 return price||jsonb_build_object('coupon_amount',discount,'total_amount',(price->>'total_amount')::bigint-discount);
end $$;
revoke all on function ss_admin.special_coupon_price(uuid,uuid,timestamptz,timestamptz,int,jsonb) from public,anon,authenticated;
...
begin
 ...
 if p_action='member_home' then
  ...
  if found then
   ...
   if b.special_coupon_id is distinct from nullif(p->>'coupon_id','')::uuid or b.starts_at<>st or b.ends_at<>en or b.guests<>(p->>'guests')::int or (b.coupon_slot is not null)<>coalesce((p->>'use_coupon')::boolean,false) or b.note<>coalesce(p->>'note','') then raise exception 'CONFLICT'; end if;
   return to_jsonb(b);
  end if;
  ...
  if nullif(p->>'coupon_id','') is not null then price:=ss_admin.special_coupon_price((p->>'coupon_id')::uuid,u.id,st,en,(p->>'guests')::int,price); end if;
  insert into ss_admin.bookings(special_coupon_id,id,member_id,starts_at,ends_at,guests,tier,discount_percent,base_amount,discount_amount,coupon_amount,total_amount,coupon_month,coupon_slot,note)
   values(nullif(p->>'coupon_id','')::uuid,(p->>'id')::uuid,u.id,st,en,(p->>'guests')::int,m.tier,(price->>'discount_percent')::int,(price->>'base_amount')::bigint,(price->>'discount_amount')::bigint,(price->>'coupon_amount')::int,(price->>'total_amount')::bigint,case when slot is not null then cm end,slot,coalesce(p->>'note','')) returning * into b;
