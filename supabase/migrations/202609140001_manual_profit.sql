-- Existing accounts and ledger entries are preserved. Run in qhvwwdrwfzwpehfjntbv.
begin;
create table if not exists ss_admin.period_profits (
...
alter table ss_admin.period_profits enable row level security;
revoke all on ss_admin.period_profits from public,anon,authenticated;
...
declare
 ...
 pr ss_admin.period_profits; old_pr ss_admin.period_profits;
...
begin
 ...
 if p_action='list' then
  ...
  select * into pr from ss_admin.period_profits where period_start=d1 and period_end=d2;
  profit:=pr.amount; fee:=round(profit*0.05);
  ...
  return jsonb_build_object('entries',vals,'count',n,'summary',jsonb_build_object('spacecloud',sc,'invoice',inv,'cash',cash,'fixed',fixed,'expense',expense,
   'revenue',sc+inv+cash,'spending',fixed+expense,'profit_mode','manual','profit_version',coalesce(pr.version,0),'profit',profit,'fee',fee,'settlement',inv+cash-expense-fee));
 ...
 if p_action='save_profit' then
  ...
  perform pg_advisory_xact_lock(hashtextextended('profit:'||d1::text||':'||d2::text,0));
  select * into old_pr from ss_admin.period_profits where period_start=d1 and period_end=d2 for update;
  if coalesce(old_pr.version,0)<>(p->>'version')::int then raise exception 'CONFLICT'; end if;
  insert into ss_admin.period_profits(period_start,period_end,amount,updated_by)
   values(d1,d2,(p->>'amount')::bigint,u.id)
   on conflict(period_start,period_end) do update set amount=excluded.amount,updated_by=u.id,updated_at=now(),version=ss_admin.period_profits.version+1
   returning * into pr;
  insert into ss_admin.audit(actor_id,action,target_id,before_data,after_data)
   values(u.id,'profit_saved',pr.id,case when old_pr.id is null then null else to_jsonb(old_pr) end,to_jsonb(pr));
