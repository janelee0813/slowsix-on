-- Invitation-only membership. Apply after 202609150003. Existing finance stays separate.
begin;
alter table ss_admin.people drop constraint if exists people_role_check;
alter table ss_admin.people add constraint people_role_check check(role in ('admin','operator','member'));
alter table ss_admin.links drop constraint if exists links_kind_check;
alter table ss_admin.links add constraint links_kind_check check(kind in ('bootstrap','invite','reset','member'));
alter table ss_admin.links add column if not exists tier text check(tier in ('friends','crew'));
create table if not exists ss_admin.members (
 person_id uuid primary key references ss_admin.people(id), tier text not null check(tier in ('friends','crew')),
 gender text not null default '', age_group text not null default '', purposes jsonb not null default '[]', version int not null default 1
);
create table if not exists ss_admin.bookings (
 id uuid primary key, member_id uuid not null references ss_admin.members(person_id),
 starts_at timestamptz not null, ends_at timestamptz not null check(ends_at>starts_at), guests int not null check(guests between 6 and 13),
 tier text not null, discount_percent int not null, base_amount bigint not null, discount_amount bigint not null,
 coupon_amount int not null default 0, total_amount bigint not null check(total_amount>=0),
 coupon_month date, coupon_slot int check(coupon_slot in (1,2)),
 status text not null default 'requested' check(status in ('requested','awaiting_payment','confirmed','rejected','cancelled')),
 note text not null default '', payment_note text not null default '', admin_note text not null default '',
 version int not null default 1, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create unique index if not exists coupon_once on ss_admin.bookings(member_id,coupon_month,coupon_slot) where coupon_slot is not null and status in ('requested','awaiting_payment','confirmed');
create index if not exists booking_times on ss_admin.bookings(starts_at,ends_at);
create table if not exists ss_admin.booking_blocks (
 id uuid primary key, starts_at timestamptz not null, ends_at timestamptz not null check(ends_at>starts_at),
 note text not null default '', created_by uuid not null references ss_admin.people(id), created_at timestamptz not null default now()
);
create table if not exists ss_admin.member_notifications (
 id bigint generated always as identity primary key, recipient uuid not null references ss_admin.people(id),
 message text not null, booking_id uuid references ss_admin.bookings(id), read_at timestamptz, created_at timestamptz not null default now()
);
create table if not exists ss_admin.membership_settings (id boolean primary key default true check(id),payment_note text not null default '');
insert into ss_admin.membership_settings(id) values(true) on conflict do nothing;
alter table ss_admin.members enable row level security;
alter table ss_admin.bookings enable row level security;
alter table ss_admin.booking_blocks enable row level security;
alter table ss_admin.member_notifications enable row level security;
alter table ss_admin.membership_settings enable row level security;
revoke all on ss_admin.members,ss_admin.bookings,ss_admin.booking_blocks,ss_admin.member_notifications,ss_admin.membership_settings from public,anon,authenticated;
-- Preserve the financial gateway once. Never re-run older gateway migrations after this update.
do $$ begin
 if to_regprocedure('ss_admin.finance_gateway(text,jsonb)') is null then
  alter function public.ss_admin_gateway(text,jsonb) rename to finance_gateway;
  alter function public.finance_gateway(text,jsonb) set schema ss_admin;
 end if;
end $$;
revoke all on function ss_admin.finance_gateway(text,jsonb) from public,anon,authenticated,service_role;
-- Honor the Edge session allowance (600 / 15 minutes); login remains capped at 10.
do $$ begin
 execute replace(pg_get_functiondef('ss_admin.finance_gateway(text,jsonb)'::regprocedure),
 'cap := least(100,greatest(1,','cap := least(600,greatest(1,');
end $$;

create or replace function ss_admin.booking_price(st timestamptz,en timestamptz,guests int,tier text,use_coupon boolean)
returns jsonb language plpgsql stable set search_path=pg_catalog,ss_admin as $$
declare base bigint; pct int; discounted bigint;
begin
 if st is null or en is null or guests is null or guests not between 6 and 13 or tier is null or tier not in ('friends','crew') or en<=st or en-st>interval '24 hours' or
 date_trunc('hour',st)<>st or date_trunc('hour',en)<>en then raise exception 'INVALID_BOOKING'; end if;
 select sum(case when extract(isodow from h at time zone 'Asia/Seoul') in (6,7) then 2500 else 2000 end)*guests into base from generate_series(st,en-interval '1 hour',interval '1 hour') h;
 pct:=case tier when 'crew' then 20 else 12 end; discounted:=base*(100-pct)/100;
 return jsonb_build_object('base_amount',base,'discount_percent',pct,'discount_amount',base-discounted,'coupon_amount',case when use_coupon then 3000 else 0 end,'total_amount',greatest(0,discounted-case when use_coupon then 3000 else 0 end),'tier',tier);
end $$;
revoke all on function ss_admin.booking_price(timestamptz,timestamptz,int,text,boolean) from public,anon,authenticated;
...
declare u ss_admin.people; l ss_admin.links; m ss_admin.members; b ss_admin.bookings; old_b ss_admin.bookings;
 result jsonb; vals jsonb; price jsonb; st timestamptz; en timestamptz; cm date; slot int; quota int; used int; target_status text;
begin
 if p_action in ('link_info','register') then
  ...
  if l.kind='member' then
   ...
   insert into ss_admin.people(id,username,name,auth_email,role,status) values((p->>'id')::uuid,p->>'username',p->>'name',p->>'email','member','active') returning * into u;
 ...
 if p_action='member_home' then
  ...
  if u.role='admin' then return jsonb_build_object('notifications',vals,'pending',(select count(*) from ss_admin.bookings where status in ('requested','awaiting_payment'))); end if;
  ...
  if p_action='member_calendar' then
   select coalesce(jsonb_agg(to_jsonb(t) order by t.starts_at),'[]') into vals from (
    select starts_at,ends_at from ss_admin.bookings where status in ('requested','awaiting_payment','confirmed') and starts_at<en and ends_at>st
    union all select starts_at,ends_at from ss_admin.booking_blocks where starts_at<en and ends_at>st)t;
  else
   select coalesce(jsonb_agg(to_jsonb(t) order by t.starts_at),'[]') into vals from (
    ...
    where (u.role='admin' or booking.member_id=u.id) and booking.starts_at<en and booking.ends_at>st)t;
  end if;
  ...
  if p_action='member_quote' then return price; end if;
  ...
  if found then
   ...
   if b.starts_at<>st or b.ends_at<>en or b.guests<>(p->>'guests')::int or (b.coupon_slot is not null)<>coalesce((p->>'use_coupon')::boolean,false) or b.note<>coalesce(p->>'note','') then raise exception 'CONFLICT'; end if;
   return to_jsonb(b);
  end if;
  if exists(select 1 from ss_admin.bookings where status in ('requested','awaiting_payment','confirmed') and starts_at<en and ends_at>st) or exists(select 1 from ss_admin.booking_blocks where starts_at<en and ends_at>st) then raise exception 'TIME_UNAVAILABLE'; end if;
  ...
  insert into ss_admin.bookings(id,member_id,starts_at,ends_at,guests,tier,discount_percent,base_amount,discount_amount,coupon_amount,total_amount,coupon_month,coupon_slot,note)
   values((p->>'id')::uuid,u.id,st,en,(p->>'guests')::int,m.tier,(price->>'discount_percent')::int,(price->>'base_amount')::bigint,(price->>'discount_amount')::bigint,(price->>'coupon_amount')::int,(price->>'total_amount')::bigint,case when slot is not null then cm end,slot,coalesce(p->>'note','')) returning * into b;
  insert into ss_admin.member_notifications(recipient,message,booking_id) select id,u.name||'님의 새 예약 요청이 도착했습니다.',b.id from ss_admin.people where role='admin';
  return to_jsonb(b);
  ...
  perform pg_advisory_xact_lock(992301);
  select * into m from ss_admin.members where person_id=(p->>'id')::uuid for update;
  ...
  if exists(select 1 from ss_admin.bookings where status in ('requested','awaiting_payment','confirmed') and starts_at<en and ends_at>st) then raise exception 'TIME_UNAVAILABLE'; end if;
  insert into ss_admin.booking_blocks(id,starts_at,ends_at,note,created_by) values((p->>'id')::uuid,st,en,coalesce(p->>'note',''),u.id) on conflict(id) do nothing;return '{}'::jsonb;
