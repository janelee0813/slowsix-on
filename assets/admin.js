...
$('tab-membership').addEventListener('click',()=>busy($('tab-membership'),async()=>{const expected=session;const {mountMembershipAdmin}=await import('./membership-admin.js');if(session!==expected||user?.role!=='admin')return;if(!membershipAdmin)membershipAdmin=mountMembershipAdmin($('membership-view'),api,notice);switchTab('membership');await membershipAdmin.load();}));
setInterval(()=>{if(user?.role==='admin'&&!document.hidden)api('member_home').then(d=>{if(user?.role==='admin')$('tab-membership').textContent='멤버십 관리'+(d.pending?' ('+d.pending+')':'');}).catch(()=>{});},60000);
