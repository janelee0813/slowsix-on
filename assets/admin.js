...
$('tab-membership').addEventListener('click',()=>busy($('tab-membership'),async()=>{const expected=session;const {mountMembershipAdmin}=await import('./membership-admin.js?v=approval1');if(session!==expected||user?.role!=='admin')return;if(!membershipAdmin)membershipAdmin=mountMembershipAdmin($('membership-view'),api,notice);switchTab('membership');await membershipAdmin.load();}));
