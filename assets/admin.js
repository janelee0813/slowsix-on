...
async function enter(){
 ...
 const admin=user.role==='admin';$('role-badge').textContent=admin?'관리자':'운영자';$('tab-team').hidden=!admin;$('tab-audit').hidden=!admin;$('edit-heading').hidden=!admin;$('recurring-editor').hidden=!admin;$('recurring-permission').textContent=admin?'관리자는 고정 지출을 등록·수정할 수 있습니다.':'고정 지출 설정은 관리자만 변경할 수 있습니다.';resetRecurring();
...
function switchTab(name){for(const v of ['ledger','recurring','team','audit']){$(v+'-view').hidden=v!==name;$('tab-'+v).classList.toggle('active',v===name);}}
...
async function loadAudit(){const expected=session;const data=await api('audit');if(session!==expected||user?.role!=='admin')return;$('audit-list').replaceChildren();const titles={entry_created:'내역 등록',entry_updated:'내역 수정',entry_deleted:'내역 삭제',account_created:'계정 생성',status_changed:'계정 상태 변경',invite_created:'운영자 초대 발급',name_updated:'닉네임 변경',recurring_saved:'고정 지출 저장',recurring_deleted:'고정 지출 삭제',profit_saved:'이전 순수익 입력',fee_saved:'운영 수수료 저장'};for(const a of data.audit){const row=elem('article',undefined,'audit-row'),main=elem('div');main.append(elem('p',`${a.actor||'관리자'} · ${titles[a.action]||a.action}`));if(a.before_data||a.after_data){const detail=elem('details'),summary=elem('summary','변경 내용 보기','small muted');detail.append(summary);for(const [label,d] of [['변경 전',a.before_data],['변경 후',a.after_data]])if(d){const parts=[];if(d.entry_date)parts.push(d.entry_date,names[d.category],d.description,money(d.amount));if(d.start_month)parts.push(d.description,names[d.category],money(d.amount),d.start_month.slice(0,7)+' ~ '+(d.end_month?.slice(0,7)||'계속'));if(d.period_start)parts.push(d.period_start+' ~ '+d.period_end,money(d.amount));if(d.status)parts.push(statuses[d.status]||d.status);if(d.username)parts.push(d.username);if(d.name)parts.push(d.name);detail.append(elem('pre',label+' : '+parts.join(' · ')));}main.append(detail);}row.append(main,elem('time',time(a.created_at)));$('audit-list').append(row);}if(!data.audit.length)$('audit-list').append(elem('p','변경 이력이 없습니다.','muted'));
}
